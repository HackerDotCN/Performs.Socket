...
namespace SocketServerCommonLib
{
    ...
    public class SocketInvokeElement
    {
        ...
        public virtual bool ProcessReceive(byte[] buffer, int offset, int count) //接收异步事件返回的数据，用于对数据进行缓存和分包
        {
            ...
            {
                //命令 组装-如果组装失败则继续接收：如果组装成功-则删除sizeof(int)为4个bit
                result = ProcessPacket(receiveBuffer.Buffer, 0, packetLength);
                if (result)
        ...
        {
            string tmpStr = Encoding.ASCII.GetString(buffer, offset, count);
            //* DelegateState.ServerStateInfo("接收信息:" + tmpStr);
            //命令组装或者查询
